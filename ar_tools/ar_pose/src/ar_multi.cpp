...
namespace ar_pose
{
  ARMultiPublisher::ARMultiPublisher (ros::NodeHandle & n):n_ (n), it_ (n_)
  {
    ...
    std::string default_path = "data/object_4x4"; //gets the pattern information
    ...
    if (!n_param.getParam ("publish_tf", publishTf_)) //whether or not the transforms will be published
      publishTf_ = true;
    ROS_INFO ("\tPublish transforms: %d", publishTf_);

    if (!n_param.getParam ("publish_visual_markers", publishVisualMarkers_)) //whether or not the visual markers will be published
      publishVisualMarkers_ = true;
    ROS_INFO ("\tPublish visual markers: %d", publishVisualMarkers_);

    if (!n_param.getParam ("threshold", threshold_)) //threshold value for functions
      threshold_ = 100;
    ROS_INFO ("\tThreshold: %d", threshold_);
	
    if (!n_param.getParam ("publish_topic", publishTopic_)) //the topic that the ar poses will be published to
      publishTopic_ = "ar_pose_marker";	
    ...
    arMarkerPub_ = n_.advertise < ar_pose::ARMarkers > (publishTopic_, 0);
